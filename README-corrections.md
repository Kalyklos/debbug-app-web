...
**Bouton retour du téléphone.** Appuyer sur la flèche retour d'Android (ou faire le geste de retour) quittait purement et simplement l'application. Une entrée sentinelle est maintenant maintenue dans l'historique du navigateur : le retour ferme d'abord ce qui est ouvert (tutoriel, confirmation de victoire, panneau d'indice/machine/loupe/micro, sous-vue labyrinthe ou lecteur QR qui revient à la grille d'outils), recule d'un écran quand c'est cohérent (briefing → réglages → liste des missions, photo → victoire), et met la partie en pause si le chrono tourne. Les overlays à choix explicite (reprise de partie, pause) ne sont volontairement pas fermables par retour, pour éviter une relance de chrono ou un abandon de sauvegarde accidentels. À la racine, un premier appui affiche « Appuyez encore pour quitter » et seul un second appui dans les 2,6 secondes quitte réellement — la partie étant de toute façon sauvegardée.

Point technique important : deux protections des navigateurs mobiles rendaient le premier correctif inopérant selon le contexte de test. D'une part, Chrome sur Android **ignore les entrées d'historique ajoutées avant toute interaction de l'utilisateur** (protection contre le détournement du bouton retour) : la sentinelle est donc armée au chargement *et* réarmée à chaque geste. D'autre part, Chrome **refuse `history.pushState` quand la page est ouverte en local** (`file://` ou `content://`, par exemple un fichier lancé depuis le gestionnaire de téléchargements — c'est ce contexte qui produit le message Android « Impossible d'ouvrir l'application » au retour) : un repli sur `location.hash` prend alors le relais, car un changement de hash crée aussi une entrée d'historique, même en local. Le retour ne peut désormais plus vider l'historique : à la racine, il affiche « Pour quitter, fermez l'onglet », comportement standard des jeux web. En complément, un garde-fou `beforeunload` demande une confirmation native si le navigateur tente malgré tout de quitter la page pendant une partie en cours (ce qui couvre aussi le rafraîchissement accidentel). Cette approche a été préférée à une seconde page HTML « tampon » : revenir sur `index.html` depuis une autre page rechargerait l'application et interromprait la partie, là où la sentinelle garde tout l'état en mémoire.

**Conditions de test.** Pour un test représentatif, servir les fichiers via une vraie URL HTTPS (ou au minimum un serveur local `http://`), pas en ouvrant le fichier téléchargé directement : en `file://` ou `content://` (fichier lancé depuis le gestionnaire de téléchargements), Chrome neutralise la manipulation d'historique **quelle que soit la technique** — bouton retour irrécupérable, avec le message Android « Impossible d'ouvrir l'application » quand le système échoue à revenir vers l'application d'origine — et bloque aussi la caméra, les capteurs et le service worker. L'application détecte désormais ce contexte et affiche un bandeau d'avertissement pour éviter de conclure à un bug. Un hébergement gratuit type GitHub Pages, Netlify ou Vercel suffit pour les essais. À noter également : le service worker sert la page en réseau-d'abord, afin que chaque mise à jour de `index.html` déployée soit visible immédiatement (le cache ne prend le relais qu'en cas de coupure).
